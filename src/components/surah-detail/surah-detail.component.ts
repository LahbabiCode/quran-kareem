...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, LoadingSpinnerComponent, PlayIconComponent, PauseIconComponent, BookOpenIconComponent, SparkleIconComponent, ModalComponent, StarIconComponent, StarSolidIconComponent, BookmarkIconComponent, BookmarkSolidIconComponent, SoundWaveIconComponent, ShareIconComponent, CogIconComponent],
...
export class SurahDetailComponent {
  ...
  // --- Reciter Signals ---
  private _perAyahReciters = toSignal(
  ...
  perAyahReciters = computed(() => {
    const reciters = this._perAyahReciters();
    if (!reciters || !Array.isArray(reciters)) {
      return [];
    }
    return reciters.filter(reciter => reciter && typeof reciter === 'object');
  });
  ...
  recitersForSurahMode = computed(() => {
    const reciters = this.fullSurahReciters();
    const surahId = this.surahId();
    
    // Comprehensive defensive checks
    if (!reciters || !Array.isArray(reciters) || !surahId) {
      return [];
    }
    
    try {
      const validReciters = reciters
        .filter(reciter => reciter && typeof reciter === 'object')
        .map(reciter => {
          // Add defensive check for moshaf array
          ...
          const moshaf = reciter.moshaf.find(m => 
            ...
            m.surah_list.split(',').includes(String(surahId))
          );
          
          // Add a defensive check to ensure the moshaf and its server URL are valid
      ...
      console.error('Error in recitersForSurahMode computed:', error);
      return [];
    }
  });
  ...
  selectReciter(reciterId: string) {
    if (this.playbackMode() === 'ayah') {
      // Add defensive checks for perAyahReciters
      const reciters = this.perAyahReciters();
      if (Array.isArray(reciters)) {
        const reciter = reciters.find(r => r && r.id === reciterId) || null;
        this.selectedPerAyahReciter.set(reciter);
      }
    } else {
      const reciters = this.recitersForSurahMode();
      if (Array.isArray(reciters)) {
        const reciter = reciters.find(r => r && r.id.toString() === reciterId) || null;
